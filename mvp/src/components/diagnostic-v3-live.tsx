...
const labels: Record<string, string> = {
  ...
  v3_p7_radar: 'Percepção inicial da empresa',
  v3_p8_data: 'Dados acompanhados',
  v3_p9_evidence: 'Fontes/evidências',
};
export function DiagnosticV3Live({
  sessionId,
  question,
  answers,
  readOnly,
  ...
  const [mode, setMode] = useState<'exact' | 'range' | 'other' | 'unknown'>('range');
  ...
  function answer() {
    ...
    if (question.kind === 'impact')
      return JSON.stringify({ mode, value: impact.trim(), unit: impactUnit.trim() });
  ...
  function ready() {
    ...
    if (question.kind === 'goal')
      return !!current.trim() && !!target.trim() && !!unit.trim() && !!period.trim();
    if (question.kind === 'multi-priority') return selected.length > 0 && !!primary;
    if (question.kind === 'radar')
      return radarAreas.every(([k]) =>
        v3Statuses.includes(radar[k] as (typeof v3Statuses)[number]),
      );
  ...
  async function submit(unknown = false) {
    ...
    setMessage('Salvando…');
    try {
      const result = await saveV3Answer({
      ...
      <fieldset disabled={pending || readOnly}>
          ...
          <div className="choices">
            {question.options?.map((o) => (
              <button
                type="button"
                key={o}
                className={single === o ? 'choice selected' : 'choice'}
                onClick={() => setSingle(o)}
              >
                {o}
              </button>
            ...
            <div className="choices">
              {(
                ...
                <button
                  type="button"
                  key={m}
                  className={mode === m ? 'choice selected' : 'choice'}
                  onClick={() => setMode(m)}
                >
                  {l}
                </button>
              ...
              <>
                <label className="field">
                  ...
                  <input
                    ...
                    placeholder="Ex.: 10.000 ou 5.000–10.000"
                  />
                </label>
                <label className="field">
                  Unidade
                  <input value={impactUnit} onChange={(e) => setImpactUnit(e.target.value)} />
                </label>
              </>
            )}
          </div>
          ...
          <div className="stack">
            ...
            <label className="field">
              ...
              <input
                ...
                placeholder="Ex.: R$, clientes, dias"
              />
            </label>
            <label className="field">
              Período
            ...
            <p className="caption">
              Evite abreviações como “50 mi”. Escreva o valor e a unidade completos.
            </p>
          </div>
        )}
        ...
        {question.kind === 'radar' && (
          <div className="stack">
                ...
                <div className="choices">
                  {v3Statuses.map((s) => (
                    <button
                      type="button"
                      key={s}
                      className={radar[k] === s ? 'choice selected' : 'choice'}
                      onClick={() => setRadar((v) => ({ ...v, [k]: s }))}
                    >
                      {s}
                    </button>
                  ))}
            ...
            <p className="caption">
              Verde = parece funcionar bem · Amarelo = merece atenção · Vermelho = vejo problema
              importante · Cinza = não acompanho/não sei avaliar.
            </p>
          </div>
        )}
        {question.kind === 'data' && (
            ...
            <div className="panel">
              ...
              {prior.map((a) => (
                <div className="list-row" key={a.question_id}>
                  <strong>{labels[a.question_id] ?? a.question_id}</strong>
                  <span>{displayV3Answer(a)}</span>
                </div>
              ))}
            </div>
            <p className="muted">
              Confirmar significa que o resumo representa o que você quis informar. Percepções
              continuam sendo percepções até serem validadas por dados.
            </p>
          </div>
        )}
        <div className="row">
